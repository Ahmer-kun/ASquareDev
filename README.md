# A² Dev Portfolio

Personal portfolio of Muhammad Ahmer (A² Dev), a MERN Stack and Web3 developer. The site features project showcases, technical writing, and development services.

Live: [asquardev.vercel.app](https://asquardev.vercel.app/)

## Built With

- React 19 with TypeScript
- Tailwind CSS
- React Router DOM
- EmailJS
- Vite

## Features

- Responsive design for all devices
- Dark theme with glass morphism effects
- Live GitHub stats widget on the homepage
- Project gallery with live demo links
- Technical blog section
- Contact form with EmailJS integration
- Dynamic page titles for SEO
- Smooth scroll animations
- Live status display with UTC+5 local time

## GitHub Statistics

| Metric | Value |
|--------|-------|
| Username | Ahmer-kun |
| Public Repositories | 40+ |
| Total Commits (2026) | 500+ |
| Primary Languages | TypeScript, JavaScript, HTML, CSS |
| Core Stack | MERN, Web3, React, Solidity |

## Recent Development Activity

- Building responsive web applications with React and Tailwind CSS
- Exploring Web3 and blockchain integration
- Developing Chrome extensions using Manifest V3
- Creating full-stack applications with Node.js and MongoDB

## Connect

- GitHub: [github.com/Ahmer-kun](https://github.com/Ahmer-kun)
- LinkedIn: [Muhammad Ahmer](https://www.linkedin.com/in/muhammad-ahmer-b88485283/)
- Email: muhammadahmer1qw2@gmail.com

## Credits

- Stock images from Unsplash
- Icons from Font Awesome
- Font from Google Fonts (Plus Jakarta Sans)